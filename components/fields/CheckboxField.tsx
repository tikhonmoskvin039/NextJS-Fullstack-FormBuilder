...
const DesignerComponent = ({ elementInstance }: {elementInstance: FormElementInstance}) => {
  ...
  return (
    <div className='flex items-top space-x-2'>
        <Checkbox id={id}/>
        <div className='grid grap-1.5 leading-none'>
...
const FormComponent = ({ elementInstance, submitValue, isInvalid, defaultValue }:
  ...
  return (
    <div className='flex items-top space-x-2'>
        ...
        />
        <div className='grid grap-1.5 leading-none'>
            <Label htmlFor={id} className={cn(error && 'text-red-500')}>{label}
                {required && '*'}
            </Label>

            {helperText && <p className={cn(error && 'text-red-500',
              'text-muted-foreground text-[0.8rem]')}>{helperText}</p>}
        </div>
    </div>
  )
}
